...
def get_holiday_calendar():
    ...
    return holidays.China(years=list(range(current_year - 2, current_year + 5)))


def is_trading_day(day: date) -> bool:
    return day.weekday() < 5 and day not in get_holiday_calendar()
...
def _get_session_windows_for_date(
    ...
    if profile.get("has_night"):
        ...
        if next_calendar_day not in get_holiday_calendar():
